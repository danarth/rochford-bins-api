...
class BinsAndCollectionsCalendarPage:
    ...
    async def navigate(self):
        await self.page.goto("https://www.rochford.gov.uk/online-bin-collections-calendar")

    async def get_bin_collection_dates(self):
        ...
        for table in await self.bin_collection_tables.all():
            ...
            if match := month_year_regex.match(header_text):
                ...
                for row in rows:
                    all_records += await self._expand_bin_collection_table_row(row, year)
        return all_records

    async def _expand_bin_collection_table_row(self, row: Locator, year: str) -> list[BinCollectionDate]:
        """
        ...
        """
        date_range = await row.locator(":nth-match(td, 1)").text_content()
        bin_type = await row.locator(":nth-match(td, 2)").text_content()
